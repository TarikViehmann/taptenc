/** \file
 * Provides datastructures to store visual information on timed automata
 * systems, such as state and transition coordinates.
 *
...
private:
  /** Tracks the number of transitions that have the same source and dest.*/
  ::std::vector<
  ...
  ///@{
  /**
   * Stores the generated state and transition info for each TA of a system.
   *
  ...
  /**
   * Generates coordinates for the given states.
   *
  ...
  /**
   * Generates coordinates for the given states with an offset.
   *
   * @param states states to generate generate coordinates for
   * @param x_offset x offset to add, stores the x coordinate of the rightmost
   *                 placed state afterwards
   * @param y_offset y offset to add, stores the y coordinate of the bottom-most
   *                 placed state afterwards
   * @return state id -> StateVisInfo mapping
   */
  ::std::unordered_map<::std::string, StateVisInfo>
...
public:
  ...
  /**
   * Generates all necessary visual information from a given automata system.
   *
  ...
  /**
   * Gets the endpoint positions of a transition.
   *
   * @param component_index TA index corresponding to the TA index in the
   *                        associated AutomataSystem
